...
**Live-refresh readiness (shipped, groundwork for future manual editing):** the user has confirmed manual schedule editing (reassigning an NPC's enrolled classes) is coming eventually. Since every show* function above already rebuilds its HTML fresh from `gameState` on each call — nothing is cached — the only missing piece was a way to re-render whatever schedule view is *currently on screen* after an off-screen mutation. `scheduleModalContext` now tracks which view is open (teacher grid / student grid / roster / lookup) and `refreshOpenScheduleModal()` re-renders it in place; it's already wired into both existing mutation points (`hireFacultyCandidate()`, `fireFaculty()`). A future manual-enrollment-editing feature only needs to call `refreshOpenScheduleModal()` after mutating `gameState.sections`/`scheduledSectionIds` to keep every open view honest, without needing to know which view is open or how to rebuild it. Also fixed as part of this: `fireFaculty()` previously left the teacher's sections orphaned in `gameState.sections` (a pre-existing gap, not introduced by the schedule UI) — it now removes them and cleans up affected students' `scheduledSectionIds` too, since a dangling section would otherwise silently break the "faithful schedule" guarantee the moment a teacher was dismissed.
